...
def main():
    ...
    ap.add_argument("--out", default="/mnt/data/xi_overlap_finiteE_report.json")
    args = ap.parse_args()
    ...
    #   SFV_positive: integrate toward +xi (increasing r relative to Rpk)
    #   SFV_negative: integrate toward -xi (decreasing r relative to Rpk)
    ...
    Lwkb = float(P.get("xi_cutoff_mult",3.0))*FWHM
    xi_end = float(np.clip(xi_star + sgn*Lwkb, xi.min(), xi.max()))
    # Build a fine grid to integrate
    def I_of_E(E):
        # sample points from xi* toward the chosen side on the available domain
        xgrid = np.linspace(xi_star, xi_end, 1200)
        k = np.array([kappa(z,E) for z in xgrid])
        return float(abs(np.trapz(k, xgrid)))
    ...
    out = {
        "inputs": {
            ...
            "xi_star": xi_star, "xi_side": side, "xi_end": xi_end, "phi_star": phi_star, "Delta_prime": Delta_prime
        },
